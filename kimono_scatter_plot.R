...
aggregatedData <- read.csv("/Users/ko/Documents/mom/kimono/raw_data/student_response_aggregated.csv", header=TRUE)
...
aggregatedPlotSplit <- ggplot(aggregatedData, aes(x = Nihon, y = Basic, color = factor(Type))) +
  # geom_point(aes(color = factor(Type))) +
...
sumByType <- read.csv("/Users/ko/Documents/mom/kimono/raw_data/student_response_sum_by_type.csv", header=TRUE)
